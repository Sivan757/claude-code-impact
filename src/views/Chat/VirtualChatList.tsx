...
export function VirtualChatList({
  ...
  return (
    <div ref={parentRef} onScroll={handleScroll} className="h-[calc(100vh-320px)] overflow-auto -mr-4 pr-4">
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded ${chat.role === "user" ? "bg-primary/15 text-primary" : "bg-card-alt text-muted-foreground"
                      }`}
                  >
                    {chat.role}
                  </span>
                ...
                <p className="text-xs text-muted-foreground mt-2 truncate">
                  {formatPath(chat.project_path)} · {chat.session_summary || t('chat.untitled_session')}
                </p>
              </button>
            </div>
          );
      ...
      {loadingMore && <div className="py-4 text-center text-muted-foreground text-sm">{t('chat.loading_more')}</div>}
    </div>
  );
}

    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!flex !flex-col max-w-2xl max-h-[80vh] overflow-hidden">
        ...
        <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 py-2 border-b border-border items-center">
          <Label className="text-sm text-muted-foreground-foreground">{t('export.select')}</Label>
          <Select value={selectPreset} onValueChange={(v) => setSelectPreset(v as "all" | "user" | "custom")}>
            <SelectTrigger size="sm">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('export.all', { count: allMessages.length })}</SelectItem>
              <SelectItem value="user">{t('export.user', { count: allMessages.filter((m) => m.role === "user").length })}</SelectItem>
            ...
            <>
              ...
              <Select value={mdStyle} onValueChange={(v) => setMdStyle(v as MarkdownStyle)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="bullet">{t('export.bullet')}</SelectItem>
                </SelectContent>
              </Select>
            </>
          )}

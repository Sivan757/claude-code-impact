...
export function MessageView({ projectId, projectPath, sessionId, summary: initialSummary, onBack }: MessageViewProps) {
  ...
  const displaySummary = toReadable(freshSummary) || t('message_view.session');
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-muted-foreground">{t('message_view.loading')}</p>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        {filteredMessages.map((msg) => {
          ...
          return (
            <div
              key={msg.uuid}
              className={`group relative rounded-xl p-4 ${msg.role === "user" ? "bg-card-alt" : "bg-card border border-border"
                }`}
            >
              <DropdownMenu>
                <DropdownMenuTrigger asChild>

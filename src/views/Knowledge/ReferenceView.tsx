...
export function ReferenceView({
  ...
  if (loading) return <LoadingState message={t('reference.loading_sources')} />;
    ...
    <ConfigPage>
      <div className="space-y-2">
        {sources.length > 0 ? (
            ...
            <div key={source.name}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{t('reference.docs_count', { count: source.doc_count })}</div>
                </div>
                <ChevronDownIcon
              ...
              {expandedSource === source.name && (
                <div className="ml-4 mt-1 space-y-1">
                  {docsLoading ? (
                    ...
                    <div className="px-4 py-2 text-sm text-muted-foreground">{t('reference.no_docs')}</div>
                  )}
                </div>
              )}
            </div>
          ...
          <EmptyState
            icon={BookmarkIcon}
            message={t('reference.no_sources')}
            hint={t('reference.no_sources_hint')}
          />
        )}
      </div>
    </ConfigPage>

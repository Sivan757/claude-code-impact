...
export function ProjectDashboard({ project }: ProjectDashboardProps) {
  ...
  const handleResumeSession = async (session: Session) => {
    ...
    setWorkspace((currentWorkspace) => {
      ...
      const title = toReadable(session.summary) || t('common.untitled');
        ...
        <div className="grid grid-cols-12 gap-4 h-full" style={{ minHeight: '600px' }}>
          ...
          {recentSessions.length > 0 && (
            <div className="col-span-12 bg-card border border-border rounded-2xl px-4 py-3">
              <h3 className="text-xs font-medium text-muted-foreground mb-2">{t('workspace.recent_sessions')}</h3>
              <div className="flex gap-2 overflow-x-auto pb-1">
                {recentSessions.map((session) => (
                  ...
                  >
                    ...
                    <span className="truncate max-w-[200px]">{toReadable(session.summary) || t('common.untitled')}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="col-span-8 row-span-2">
            <BentoCard
              title={t('workspace.sessions_title')}
              className="h-full"
              subtitle={
                <span className="text-[10px] text-muted-foreground">
                  {isLoading ? "..." : t('chat.session_count', { count: filteredSessions.length })}
                </span>
              }
            >
              {isLoading ? (
                <div className="h-full flex items-center justify-center">
                  <p className="text-muted-foreground text-sm">{t('workspace.loading_sessions')}</p>
                </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm truncate">
                            {toReadable(session.summary) || t('common.untitled')}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {t('chat.message_count', { count: session.message_count })} · {formatDate(session.last_modified)}
                          </p>
                        </div>
                      </button>
                      <DropdownMenu>
                ...
                <div className="h-full flex items-center justify-center">
                  <p className="text-muted-foreground text-sm">{t('workspace.no_sessions_yet')}</p>
                </div>
              )}
            </BentoCard>
          </div>

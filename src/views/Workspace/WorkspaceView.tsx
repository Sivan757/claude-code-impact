...
export function WorkspaceView() {
  ...
  const handlePanelSplit = useCallback(
    (targetPanelId: string, direction: "horizontal" | "vertical") => {
      ...
      const newPanel: StoredPanelState = {
        id: panelId,
        sessions: [{ id: sessionId, pty_id: ptyId, title: t('common.untitled') }],
  ...
  const handleSessionAdd = useCallback(
    (panelId: string) => {
      ...
      const sessionId = crypto.randomUUID();
      const ptyId = crypto.randomUUID();
      const newSession: StoredSessionState = { id: sessionId, pty_id: ptyId, title: t('common.untitled') };
      const projectId = activeProject.id;
  ...
  if (loading) {
    return (
      <div className="h-full flex items-center justify-center bg-canvas">
        <p className="text-muted-foreground">{t('workspace.loading_workspace')}</p>
      </div>
    );
  }
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <h2 className="font-serif text-2xl font-bold text-ink mb-2">
              {t('workspace.home_welcome')}
            </h2>
            <p className="text-muted-foreground mb-6">
              {t('workspace.home_tagline')}
            </p>
            <button
              onClick={handleAddProject}
              className="px-6 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors"
            >
              {t('workspace.add_first_project')}
            </button>
          </div>
        </div>
      )}

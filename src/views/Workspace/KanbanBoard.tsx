...
function FeatureCard({ feature, onClick, isDragging }: FeatureCardProps) {
  return (
    <div
      onClick={onClick}
      className={`p-3 bg-card border border-border rounded-lg cursor-pointer transition-all ${isDragging ? "opacity-50 shadow-lg" : "hover:border-primary/50 hover:shadow-sm"
        }`}
    >
...
export function KanbanBoard({
  ...
  const COLUMNS: { id: FeatureStatus; label: string }[] = useMemo(() => [
    { id: "pending", label: t('workspace.pending') },
    { id: "running", label: t('workspace.running') },
    { id: "needs-review", label: t('workspace.needs_review') },
    { id: "completed", label: t('workspace.completed') },
  ], [t]);
  ...
  const featuresByStatus = useMemo(() => COLUMNS.reduce(
    (acc, col) => {

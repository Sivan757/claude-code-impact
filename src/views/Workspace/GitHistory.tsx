...
export function GitHistory({ projectPath, features = [], onRefresh, embedded = false }: GitHistoryProps) {
  ...
  const handleRevert = async (commit: CommitInfo) => {
    try {
      ...
      alert(t('common.failed_with', { error: String(e) }));
    }
  };
      ...
      {!embedded && (
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-muted/30">
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-medium text-ink">{t('workspace.git_history')}</h3>
          ...
          <div key={feat}>
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">
                ({t('workspace.logo_manager.versions_count', { count: commits.length })})
              </span>
            </button>
            {isExpanded && (
        ...
        <div>
          ...
          >
            ...
            <span className="text-sm font-medium text-muted-foreground italic">
              {t('common.unassociated')}
            </span>
            <span className="text-xs text-muted-foreground">
              ({t('workspace.logo_manager.versions_count', { count: unassociated.length })})
            </span>
          </button>
          {expandedFeats.has("__unassociated__") && (
            <div className="pl-8 pr-4 pb-2">

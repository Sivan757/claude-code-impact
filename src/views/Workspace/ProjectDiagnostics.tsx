...
export function ProjectDiagnostics({ projectPath, embedded = false }: ProjectDiagnosticsProps) {
  ...
  const getEnvSummary = () => {
    ...
    return issues.length > 0 ? issues.join(", ") : t('workspace.diagnostics.no_issues');
  };
  ...
  return (
    <div className={embedded ? "" : "flex-shrink-0 border-b border-border"}>
      {!embedded && (
        <div className="flex items-center justify-between px-6 py-2 bg-muted/30">
          ...
          >
            ...
            {t('workspace.diagnostics.run_all')}
          </button>
        </div>
      )}
      ...
      <div className={embedded ? "" : "bg-card"}>
        ...
        <DiagnosticSection
          title={t('workspace.diagnostics.code_quality')}
          isExpanded={expandedSections.has("code-quality")}
          onToggle={() => toggleSection("code-quality")}
        >
          <DiagnosticItem
            title={t('workspace.diagnostics.tech_stack')}
            ...
            {techStack && (
              <div className="space-y-1.5 text-xs">
                ...
                {techStack.package_manager && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('workspace.diagnostics.package_manager')}</span>
            ...
            {envResult && (
              <div className="space-y-2 text-xs">
                ...
                {envResult.missing_keys.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      >
                        {t('workspace.diagnostics.add_to_env')}
                      </button>
                    </div>
                ...
                {envResult.missing_keys.length === 0 && envResult.leaked_secrets.length === 0 && (
                  <div className="text-green-600">{t('workspace.diagnostics.no_issues')}</div>
                )}
              </div>
            )}
          </DiagnosticItem>
          ...
          <DiagnosticItem
            title={t('workspace.diagnostics.file_lines')}
          ...
          <DiagnosticItem
            title={t('workspace.diagnostics.hardcoded_values')}
        ...
        <DiagnosticSection
          title={t('workspace.diagnostics.brand_system')}
          isExpanded={expandedSections.has("brand-system")}
          onToggle={() => toggleSection("brand-system")}
        >
          <DiagnosticItem
            title={t('workspace.diagnostics.brand_consistency')}
        ...
        <DiagnosticSection
          title={t('workspace.diagnostics.design_system')}
          isExpanded={expandedSections.has("design-system")}
          onToggle={() => toggleSection("design-system")}
        >
          <DiagnosticItem
            title={t('workspace.diagnostics.design_tokens')}

      ...
      <div className="p-3 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-ink">
              {currentLogo ? t('workspace.logo_manager.current_logo') : t('workspace.logo_manager.no_logo')}
            </p>
            <p className="text-xs text-muted-foreground">
              {t('workspace.logo_manager.versions_count', { count: logoVersions.length })}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {t('workspace.logo_manager.generate')}
          </button>
          <button
        ...
        {logoVersions.length > 1 && (
          <div className="space-y-1.5">
            <p className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('workspace.logo_manager.history')}</p>
        ...
        <Dialog open={showGenDialog} onOpenChange={(open) => {
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>{t('workspace.logo_manager.generate')}</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-muted/50 rounded-lg border border-border">
                <p className="text-xs text-muted-foreground">
                  {t('workspace.logo_manager.auto_info_prefix')} <span className="text-primary font-medium">{t('workspace.logo_manager.style_lovinsp')}</span> {t('workspace.logo_manager.logo_for')} <span className="font-medium text-ink">{projectName}</span>
                </p>
              </div>
              ...
              <div>
                ...
                >
                  {useCustomPrompt ? t('workspace.logo_manager.hide_custom') : t('workspace.logo_manager.show_custom')}
                </button>
              ...
              {genPreviews.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-muted-foreground">
                      {t('workspace.logo_manager.variants_info', { count: genPreviews.length })}
                    </p>
                    <button
                      onClick={() => {
                        setGenPreviews([]);
                    ...
                    >
                      {t('workspace.logo_manager.clear_all')}
                    </button>
                  </div>
              ...
              <div className="flex gap-2">
                {genPreviews.length > 0 ? (
                  <>
                    ...
                    >
                      {generating ? (
                        <>
                          ...
                          {t('workspace.logo_manager.generating')}
                        </>
                      ) : (
                        t('workspace.logo_manager.regenerate')
                      )}
                    </button>
                    <button
                    ...
                    >
                      {t('workspace.logo_manager.save_selected')}
                    </button>
                  </>
                ) : (
                  <button
                  ...
                  >
                    {generating ? (
                      <>
                        <span className="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin" />
                        {t('workspace.logo_manager.generating_variants', { count: 2 })}
                      </>
                    ) : (
                      t('workspace.logo_manager.generate')
                    )}

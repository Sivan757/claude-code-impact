...
export function FeatureTabs({
  ...
  const STATUS_OPTIONS: { value: FeatureStatus; label: string; icon: React.ReactNode }[] = useMemo(() => [
    { value: "pending", label: t('workspace.pending'), icon: <TimerIcon className="w-3.5 h-3.5 text-muted-foreground" /> },
    { value: "running", label: t('workspace.running'), icon: <UpdateIcon className="w-3.5 h-3.5 text-blue-500" /> },
    { value: "completed", label: t('workspace.completed'), icon: <CheckCircledIcon className="w-3.5 h-3.5 text-green-500" /> },
    { value: "needs-review", label: t('workspace.needs_review'), icon: <ExclamationTriangleIcon className="w-3.5 h-3.5 text-amber-500" /> },
  ], [t]);
    ...
    <>
          ...
          return (
            <ContextMenu key={feature.id}>
              <ContextMenuTrigger asChild>
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg cursor-pointer transition-colors shrink-0 ${isActive
              ...
              <ContextMenuContent className="min-w-[160px]">
                ...
                >
                  ...
                  <span>{feature.pinned ? t('workspace.unpin') : t('workspace.pin')}</span>
                </ContextMenuItem>
                <ContextMenuSeparator />
      ...
      <Dialog open={archiveAction !== null} onOpenChange={(open) => !open && handleCancelDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {archiveAction?.type === "complete" ? t('workspace.completed') : t('common.cancel')} "{pendingFeature?.name}"
            </DialogTitle>
          </DialogHeader>
          <textarea
            ref={archiveNoteRef}
            ...
            placeholder={t('workspace.optional_note')}
          ...
          <DialogFooter>
            ...
            >
              {archiveAction?.type === "complete" ? t('workspace.complete_feature') : t('workspace.cancel_feature')}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={nameDialog !== null} onOpenChange={(open) => !open && handleCancelNameDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {nameDialog?.type === "add" ? t('workspace.new_feature') : t('workspace.rename_feature')}
            </DialogTitle>
          </DialogHeader>
          <input
            type="text"
            ...
            placeholder={t('workspace.feature_name')}
          ...
          <DialogFooter>
            ...
            >
              {nameDialog?.type === "add" ? t('common.create') : t('workspace.rename')}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

...
export function FeatureSidebar({
  ...
  const setExpandedPanels = useCallback(
    (updater: Set<string> | ((prev: Set<string>) => Set<string>)) => { // i18n-ignore
      ...
      <div className="h-full flex flex-col bg-card border-r border-border w-10">
        <div className="flex-1 flex flex-col items-center pt-2 gap-1 overflow-y-auto">
          ...
          <button
            ...
            title={t('workspace.pinned_title', { count: totalSessionsCount })}
            ...
            panel.sessions.map((session) => {
              ...
              return (
                <button
                  ...
                  title={`${session.title || t('workspace.pinned')}: ${isRunning ? t('workspace.pty_running') : t('workspace.pty_stopped')}`}
                  className={`w-2 h-2 rounded-full hover:ring-2 hover:ring-primary/50 transition-all ${isRunning ? "bg-green-500" : "bg-muted-foreground/40"
                    }`}
                />
              );
            })
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-border flex-shrink-0">
        ...
        {isRenaming ? (
          ...
          <span
            ...
            title={featureName && onFeatureRename ? t('workspace.rename_hint') : undefined}
          >
            {title}
          </span>
        )}
      ...
      <div ref={sectionsRef} className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        >
          <SectionHeader
            title={t('workspace.pinned_sessions')}
              ...
              return (
                ...
                >
                  <SessionPanel
                    ...
                    titleFallback={t('workspace.pinned')}
                  />
                </div>
              );
            })}

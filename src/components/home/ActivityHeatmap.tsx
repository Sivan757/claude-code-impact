...
export function ActivityHeatmap({ daily, detailed }: ActivityHeatmapProps) {
    ...
    weeks.forEach((week, weekIdx) => {
      ...
      if (firstValidCell) {
        ...
        if (month !== lastMonth) {
          ...
          monthLabels.push({ month: t(`home.activity.months.${monthKey}`), weekIdx });
          lastMonth = month;
        }
      }
    });
  ...
  const hourData = useMemo(() => {
    ...
    days.forEach((day, dayIdx) => {
      ...
      if (month !== lastMonth) {
        const monthKey = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"][month];
        monthLabels.push({ month: t(`home.activity.months.${monthKey}`), dayIdx });
        lastMonth = month;
      }
    });
  ...
  const cellGap = 3;
  const weekLabels = [
    t('home.activity.weekdays.sun'),
    t('home.activity.weekdays.mon'),
    t('home.activity.weekdays.tue'),
    t('home.activity.weekdays.wed'),
    t('home.activity.weekdays.thu'),
    t('home.activity.weekdays.fri'),
    t('home.activity.weekdays.sat')
  ];
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-md border border-border/60 overflow-hidden">
            <button
              ...
              className={`px-2 py-0.5 text-[10px] transition-colors ${mode === "weekday"
                  ? "bg-primary text-primary-foreground"
                  : "bg-transparent text-muted-foreground hover:bg-accent"
                }`}
            >
              {t('home.activity.mode_week')}
            </button>
            <button
              onClick={() => setMode("hour")}
              className={`px-2 py-0.5 text-[10px] transition-colors ${mode === "hour"
                  ? "bg-primary text-primary-foreground"
                  : "bg-transparent text-muted-foreground hover:bg-accent"
                }`}
            >
              {t('home.activity.mode_hour')}
            </button>
          </div>
        </div>
        <span className="text-xs text-muted-foreground">
          {totalSessions.toLocaleString()} {t('home.stats.chats')}
        </span>
      </div>
                ...
                {weekdayData.weeks.map((week, weekIdx) => (
                  <div key={weekIdx} className="flex flex-col" style={{ gap: `${cellGap}px` }}>
                    {week.map((cell, dayIdx) => (
                      <div
                        key={dayIdx}
                        className={`rounded-sm cursor-default ${cell.date ? getColorClass(cell.count, weekdayData.maxCount) : "bg-transparent"
                          }`}

  ...
  try {
    ...
    if (!cliPath) {
      issues.push({
        code: "cli-not-found",
        severity: "error",
        titleKey: "terminal.cli_not_found_title",
        titleParams: { name: commandLabel },
        detailKey: "terminal.cli_not_found_detail",
        detailParams: { kind: commandKind },
      });
    }
  } catch {
    issues.push({
    ...
    try {
      ...
      if (!hasOauth && !hasAuthToken && !hasApiKey && !hasProxyKey) {
        issues.push({
          ...
          severity: "warning",
          titleKey: "terminal.missing_credentials_title",
          detailKey: "terminal.missing_credentials_detail",
        });
      }
    } catch {
      issues.push({
        code: "missing-credentials",
        severity: "warning",
        titleKey: "terminal.unable_read_settings_title",
        detailKey: "terminal.unable_read_settings_detail",
      });
    }
  }
          ...
          if (commandText && commandKind) {
            openDiagnostic({
              ...
              issues: [{
                code: "command-failed",
                severity: "error",
                titleKey: "terminal.command_failed_title",
                detailKey: "terminal.command_failed_detail", // Note: This key doesn't exist yet, I'll add it or use error string
                detailParams: { error: String(err) },
              }],
            });
          }
          console.error("Failed to initialize PTY:", err);
          term.writeln(`\r\n\x1b[31m${t('terminal.failed_init_pty', { error: err })}\x1b[0m`);
        }
      } finally {
              ...
              if (!diagnosticShownRef.current && (preflight || (exitedQuickly && noOutput))) {
                ...
                if (exitedQuickly && noOutput) {
                  issues.push({
                    code: "command-exited",
                    severity: "error",
                    titleKey: "terminal.command_exited_title",
                    titleParams: { name: base.commandLabel },
                    detailKey: "terminal.command_exited_detail",
                  });
                }
      ...
      <Dialog open={diagnosticOpen} onOpenChange={setDiagnosticOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {diagnostic
                ? t('terminal.diag_title', { name: diagnostic.commandLabel })
                : t('terminal.diag_title_fallback')}
            </DialogTitle>
            <DialogDescription>
              {t('terminal.diag_desc_fallback')}
            </DialogDescription>
          </DialogHeader>
          {diagnostic && (
            <div className="space-y-3">
              <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                <div className="font-medium text-ink">{t('terminal.command_label')}</div>
                <code className="break-all">{diagnostic.command}</code>
                {diagnostic.cliPath && (
                  <div className="mt-2">
                    <div className="font-medium text-ink">{t('terminal.detected_cli_label')}</div>
                  ...
                  return (
                    <div key={`${issue.code}-${index}`} className="flex items-start gap-2">
                      ...
                      <div className="space-y-0.5">
                        <div className="text-sm font-medium text-ink">{(t(issue.titleKey, issue.titleParams) as string)}</div>
                        {issue.detailKey && (
                          <div className="text-xs text-muted-foreground">{(t(issue.detailKey, issue.detailParams) as string)}</div>
                        )}
                      </div>
                    </div>
                  );
          ...
          <DialogFooter>
            ...
            <Button onClick={() => setDiagnosticOpen(false)}>{t('common.cancel')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
